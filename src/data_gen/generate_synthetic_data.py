...
def build_sms_body(
    ...
    if provider == "M-Pesa":
        ...
        if tx_type == "P2P_SEND":
            return (
                f"{tx_id} Confirmed. Ksh{amount:,.2f} sent to {counterparty} "
            ...
            return (
                f"{tx_id} Confirmed. You have received Ksh{amount:,.2f} from {counterparty} "
                f"on {date_str}. New M-PESA balance is Ksh{balance:,.2f}."
            )
        elif tx_type == "CASH_IN":
            return (
                f"{tx_id} Confirmed. Received Ksh{amount:,.2f} from Agent {counterparty} "
                f"on {date_str}. New M-PESA balance is Ksh{balance:,.2f}."
            )
        elif tx_type == "CASH_OUT":
            return (
                f"{tx_id} Confirmed. Ksh{amount:,.2f} withdrawn from Agent {counterparty} "
                f"on {date_str}. New M-PESA balance is Ksh{balance:,.2f}. Transaction cost, Ksh{fee:,.2f}."
            )
        elif tx_type == "BUY_GOODS_TILL":
            return (
                f"{tx_id} Confirmed. Ksh{amount:,.2f} paid to {counterparty} "
                f"on {date_str}. New M-PESA balance is Ksh{balance:,.2f}. Transaction cost, Ksh0.00."
            )
        elif tx_type == "PAYBILL":
            return (
                f"{tx_id} Confirmed. Ksh{amount:,.2f} sent to {counterparty} AC "
            ...
            return (
                f"{tx_id} Confirmed. Ksh{amount:,.2f} paid for Airtime "
                f"on {date_str}. New M-PESA balance is Ksh{balance:,.2f}. Transaction cost, Ksh0.00."
            )
        ...
        if tx_type == "P2P_SEND":
            return (
                f"*165*S*{amt_i} RWF transferred to {counterparty} "
            ...
            return (
                f"TxID:{tx_id} Cash In of {amt_i} RWF from Agent {counterparty} "
                f"at {date_str} .Fee: 0RWF.Balance: {bal_i}RWF."
            )
        elif tx_type == "CASH_OUT":
            return (
                f"TxID:{tx_id} Cash Out of {amt_i} RWF from Agent {counterparty} "
                f"at {date_str} .Fee: {fee_i}RWF.Balance: {bal_i}RWF."
            )
        elif tx_type == "MOMOPAY_MERCHANT":
            code_str = f"({account_ref}) " if account_ref else ""
            return (
                f"TxID:{tx_id} Payment of {amt_i} RWF to {counterparty} {code_str}"
                f"successful at {date_str} .Fee: {fee_i}RWF.Balance: {bal_i}RWF."
            )
        elif tx_type == "UTILITY":
            return (
                f"TxID:{tx_id} Payment of {amt_i} RWF to {counterparty} "
